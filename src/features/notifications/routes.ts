...
export function createNotificationsRouter({ db }) {
  ...
  router.post('/notifications/:id/read', authRequired, async (req, res) => {
    const result = await db.run('UPDATE notifications SET read_at = COALESCE(read_at, CURRENT_TIMESTAMP) WHERE id = ? AND user_id = ?', req.params.id, req.user.id);
  ...
  router.post('/notifications/read-all', authRequired, async (req, res) => {
    await db.run('UPDATE notifications SET read_at = COALESCE(read_at, CURRENT_TIMESTAMP) WHERE user_id = ?', req.user.id);
    res.json({ ok: true });
  });
